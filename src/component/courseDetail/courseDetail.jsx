...
const CourseDetail = ({ courseDetail, propsValue, title, relateCourseAry, course, keyValue, titleName }) => {
      ...
      <section className="py-4 page-section-1">
        {description && (
          <div className="row p-0 m-0">
            <h1 className="head p-0 mb-3">{title} 
            {langData && <Button1 value={`change to ${lang}`} handleClick={handleChangeLang} />}
            </h1>
            <div className="p-0 detail_desc"
              // contentEditable="true"
              dangerouslySetInnerHTML={{ __html: description && description.data }}
            ></div>
          ...
          <div className="row p-0 m-0">
            <div className="col-md-12 p-0">
                  <h1 className="head p-0 mb-3">
                    Similar{" "}
                    {title == "Test Description" ? "Test Series" : title == "Test Description" ? 'Books' : "Courses"}
                  </h1>
                <FreeContent
