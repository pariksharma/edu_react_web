...
const CurrentAffair = () => {
    ...
    const currentAffairData = useSelector((state) => state.allCategory?.allCurrentAffair[0]?.data);
                ...
                <div className="col-12 col-sm-6 col-md-4 mb-3" key={index}>
                    <div className="card">
                        ...
                        <div className="m-0 bg_dark pt-3 ">
                            ...
                            <div className="m-0 blog-text" dangerouslySetInnerHTML={{__html:item.description.slice(0,150)}}></div>
                            <a role="button" className='text-decoration-none p-0 pt-2 btn readBtn' onClick={() => handleAllCA()}>Read more <i className="bi bi-arrow-right"></i></a>
                        </div>
                    </div>
                </div>
                )}
