...
import { InquiryListService, InquiryReplyListService, InquiryReplyService, InquiryService } from '@/services';
...
const showErrorToast = (toastMsg) => {
  ...
  const handleSubmit = (e) => {
    ...
    else if(versionData.country == 0 && !validateIndianNumber(inquiryForm.mobile)) {
  ...
  const handleSubmitForm = async () => {
    try{
      ...
      const response_inquiry_service = await InquiryService(encrypt(JSON.stringify(formData), token))
      const response_inquiry_data = decrypt(response_inquiry_service.data, token);
  ...
  const fetchConverstaionList = async () => {
    try{
      const formData = {}
      const response_inquiryConverstaion_service = await InquiryListService(encrypt(JSON.stringify(formData), token))
      const response_inquiryConverstaion_data = decrypt(response_inquiryConverstaion_service.data, token)
      if(response_inquiryConverstaion_data.status){
        setConversationList(response_inquiryConverstaion_data?.data.filter(item => item.type != 0));
        if(response_inquiryConverstaion_data?.data.filter(item => item.type != 0)?.length==0)
        {
          setShowError(true)
        }
      }
      ...
      console.log(response_inquiryConverstaion_data, response_inquiryConverstaion_data)
    } catch (error) {
      console.log("error found: ", error)
    }
  }
  ...
  const fetchInquiryReplyList = async () => {
    try{
      const formData = {}
      const response_inquiryReplyList_service = await InquiryReplyListService(encrypt(JSON.stringify(formData), token))
      const response_inquiryReplyList_data = decrypt(response_inquiryReplyList_service.data, token);
      ...
      console.log('response_inquiryReplyList_data', response_inquiryReplyList_data)
    } catch (error) {
      console.log("error found: ", error)
    }
  }
  ...
  const handleReplyIssue = async (id) => {
    try{
      if(reply){
        ...
        const response_inquiryReply_service = await InquiryReplyService(encrypt(JSON.stringify(formData), token));
        const response_inquiryReply_data = decrypt(response_inquiryReply_service.data, token)
          ...
          >
              ...
              <div className='card addIssue_Card px-4 py-3 mt-2'>
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="getInTouchForm mb-1">
                    ...
                    <Select
                      name="state"
                      value={InquiryType.find(InquiryType => InquiryType.value == inquiryForm.issueType) || null}
            ...
            <Tab eventKey="view" title="View Conversation">
              <div className='container-fluid p-0'>
                ...
                {!convoDetail ? 
                  conversationList?.length > 0 ? conversationList.map((item, index) => {
                  return <div className="mb-3 card conversate_card" key={index}>
                    <div className="issue_msgText">
