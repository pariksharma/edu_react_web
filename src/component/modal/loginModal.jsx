...
const LoginModal = (props) => {
  ...
  const handleSubmit = async (event, value) => {
    ...
    if (value == "login" && (mobile.length == 10 || validateEmail(email)) && password.length > 7) {
      fetchUserLogin();
    ...
    } else if ((value == "login" || value == "signupByNumber" ) && !mobile && !withEmail) {
  ...
  const classForActive = () => {
    if(userData.name && userData.email && userData.state && userData.district && userData.password && userData.confirmPassword) {
      return "btnEnabled"
    }
    else {
      return "btnDisabled"
  ...
  const fetchUserRegisterWithOTP = async () => {
    ...
    try {
      let keyform = encrypt(JSON.stringify(fonmm), token);
      if (keyform) {
        const response_userResgister_service = await userUpdateProfileService(keyform);
        const response_userRegister_data = decrypt(response_userResgister_service.data, token);
        if(response_userRegister_data?.status){
          showSuccessToast(response_userRegister_data.message);
          props.onHide();
          setTimeout(() => {
                ...
                <div className="col-md-12 col-lg-6 py-3 padding_l">
                  ...
                  {!isSignUpModal ? (
                    ...
                    <>
                      ...
                      <p className="l_text">Enter your details here to Login</p>
                      <form
                        className="logInForm"
                      ...
                      :
                        ...
                        <button
                          type="submit"
                          className={`mt-3 btn ${!withEmail ? ((mobile && (!password ?  'btnEnabled' : 'btnDisabled')) ? 'btnEnabled' : 'btnDisabled') : ((email && (!password ?  'btnEnabled' : 'btnDisabled')) ? 'btnEnabled' : 'btnDisabled')}`}
                          // disabled={!withEmail ? ((mobile && (!password ?  false : true)) ? false : true) : ((email && (!password ?  false : true)) ? false : true)}
                        >
                          Submit
                        </button>
                        {(versionData.email_show == 1 || versionData.google_login == 1) && <>
                    ...
                    !signUpWithOTP ? 
                      ...
                      <img
                        ...
                        >
                          ...
                          <button
                            type="submit"
                            className={`mt-3 btn ${((mobile.length !=10) ? 'btnDisabled' : 'btnEnabled')}`}
                      ...
                      <>
                          ...
                          <span>{versionData.country == 0 ? `${countryCode} ${mobile} `: `${mobile} `}</span>
                            <img className="ms-1 editNumLogo" src="/assets/images/editNumLogo.png" alt="" onClick={() => setGetOTP(false)} />
                          </div>
                          </p>
                          ...
                          <button
                            className={`verifyBtn btn btnDisabled ${OTP.length != 6 ? "btnDisabled" : "btnEnabled"}`}
                    ...
                    <>
                      ...
                      >
                        ...
                        <div className="col-md-12 margin_bottom">
                          <Select
                            name="state"
                            value={stateOption.find(stateOption => stateOption.value === userData.state) || null}
                        ...
                        <div className="col-md-12 margin_bottom">
                          <Select
                            name="district"
                            value={districtOption.find(districtOption => districtOption.value === userData.district) || null}
                    ...
                    !getOTP ? (
                      ...
                      <>
                        ...
                        <p className="l_text">
                          ...
                          <div className="Otp_visibleNum d-flex align-items-center">
                          <span>
                            {versionData.country == 0 ? `${countryCode} ${mobile} `: `${mobile} `}
                          </span>
                            <img className="ms-1 editNumLogo" src="/assets/images/editNumLogo.png" alt="" onClick={() => setGetOTP(false)} />
                          </div>
                        </p>
                        ...
                        <button
                          className={`verifyBtn btn ${OTP.length != 6 ? "btnDisabled" : "btnEnabled"}`}
                  ...
                  {!getOTP ? (
                    <>
                      ...
                      >
                        ...
                        <button 
                          className={`mt-5 btn ${mobile.length != 10 ? 'btnDisabled' : 'btnEnabled'}`} 
                          type="submit"
                          // disabled = {mobile.length != 10 ? true : false}
                        >
                          Get OTP
                    ...
                    <>
                      ...
                      <p className="l_text">
                        ...
                        <div className="Otp_visibleNum d-flex align-items-center">
                          ...
                          <img src="/assets/images/editNumLogo.png" alt="" onClick={() => setGetOTP(false)} />
                        </div>
                      </p>
                      <div className="otpContainer">
                        <OtpInput
