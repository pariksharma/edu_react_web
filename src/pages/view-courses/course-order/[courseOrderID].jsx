...
const CourseOrderID = () => {
  ...
  useEffect(() => {
    if(isAddressShow){
      ...
      fetchDistrictList(stateId ? stateId : stateOption.find(stateOption => stateOption.label == userData.state)?.value);
    }
  ...
  const handleBookPayment = () => {
    if(!userData.name && !userData.mobile && !userData.address && !userData.state && !userData.district && !userData.pincode){
  ...
  const handlePayment = async () => {
    try{
      ...
      if (isLoggedIn) {
        ...
        console.log('response_getPayGateway_data', response_getPayGateway_data)
        ...
        if (
          ...
          const formDataPayment =
            paymentMode == "EMI Payment" ? formDataPayment2 : formDataPayment1;
  ...
  const handleConfirmSaveAddress = async () => {
    try{
        ...
        const response_saveAddress_service = await saveAddressService(encrypt(JSON.stringify(formData), token));
        const response_saveAddress_data = decrypt(response_saveAddress_service.data, token);
  ...
  const fetchUserAddress = async () => {
    try{
      const formData = {};
      const response_getUserAddress_service = await getUserAddressService(encrypt(JSON.stringify(formData), token));
      const response_getUserAddress_data = decrypt(response_getUserAddress_service.data, token);
  ...
  const handleOptionChange = (e) => {
    ...
    const selectAddress = JSON.parse(savedAddress.filter(item => item.id == e.target.value)[0].address)
    setUserData({
      ...
      <div className="container-fluid orderContainer mt-5 mb-4">
        ...
        <div className="row mt-3">
          {!isAddressShow ? (
                      ...
                      <tr>
                        ...
                        <td>
                          <div className="py-2 d-flex align-items-center">
                            <p className="tblImg">
                              <img src={courseData?.desc_header_image ? courseData.desc_header_image : '/assets/images/noImage.jfif'} />
                            </p>
                            <p className="m-0 tbl-Title">{courseData.title}</p>
                          </div>
                        </td>
                        <td style={{ padding: "5px 5px" }}>
                          ...
                          {Number(courseData.mrp) + Number(courseData.tax) !=
                            courseData.course_sp && (
                            <>
            ...
            <div className="col-sm-12 col-md-8 col-lg-8 mb-2">
                <div className="gap-2 d-flex align-items-center">
                  ...
                  <img className="checkImg" src="/assets/images/paycheckImg.svg"
                    alt="" />
                  ...
                  >
                    ...
                    <div className="col-md-12 mb-4">
                      <label className="main" for="defaultAddress">
                        <input type="checkbox" id="defaultAddress" name="defaultAddress" value="defaultAddress" onChange={() => setDefaultAddress(!defaultAddress)} />
              ...
              <div className="col-sm-12 col-md-8 col-lg-8 mb-3 ">
                <div className="p-3 card orderCard">
                  <div className="mb-2 addNew_Address d-flex align-items-center justify-content-between">
                    ...
                    <Button2 value={"+ Add New Address"} handleClick={handleAddNewAddress} />
                  </div>
                  {savedAddress?.length > 0 && savedAddress.map((item, index) => {
                    ...
                    <p className="m-0 mobTitle">{userAddress.mobile}</p>
                  </div>
                  })}
                </div>
              </div>
          ...
          <div className="col-sm-12 col-md-4 col-lg-4 mb-3 ">
                ...
                <div className="gap-2 d-flex align-items-center mt-2">
                  ...
                  {preFillCouponList.length == 1 ? (
                    ...
                    <>
                      ...
                      />
                      <Button2 value={"View"} handleClick={handleViewCoupons} />
                    </>
                  ) : (
                    <>
                      {/* Self Fill Coupon */}
                      ...
                      courseData.course_sp && (
                        <td>
                          <p className="m-0 text-end totalAmount">
                            ...
                            {paymentMode == "One Time Payment" &&
                              (couponData
                                ? parseFloat(Number(couponData.mrp) +
                                  Number(couponData.tax)).toFixed(2)
                                : parseFloat(Number(courseData.mrp) +
                                  Number(courseData.tax)).toFixed(2))}
                          </p>
                        </td>
                      )
                      :
                            ...
                            {paymentMode == "One Time Payment" &&
                              (couponData
                                ? parseFloat(Number(couponData?.mrp) +
                                  ...
                                  Number(couponData?.delivery_charge)).toFixed(2)
                                : parseFloat(Number(courseData?.mrp) +
                                  Number(courseData?.tax) +
                                  Number(courseData?.delivery_charge)).toFixed(2))}
                          </p>
                        </td>
                      )
                    }
            ...
            <div className="row premiumIcon justify-content-center"  style={{whiteSpace: 'nowrap'}}>
